...
import { CartesiaTestClient } from "@/app/cartesia-test/cartesia-test-client";
...
export default async function CartesiaTestPage() {
  if (!(await isAdminAuthenticated())) {
    redirect("/admin?next=/cartesia-test");
  }
  ...
  return (
    <main className="relative isolate mx-auto flex min-h-screen w-full max-w-6xl flex-col gap-8 px-6 pb-5 pt-0 md:px-10">
      ...
      <header className="rounded-[2rem] bg-white/85 p-8 shadow-sm ring-1 ring-black/5 md:p-10">
        ...
        <h1 className="mt-4 max-w-3xl text-4xl font-bold tracking-tight text-ink md:text-5xl">Cartesia test page</h1>
        <p className="mt-4 max-w-3xl text-lg leading-8 text-slate-600">
          Quick TTS smoke test for comparing Cartesia Sonic against the current demo voice path before we wire it into calls.
        </p>
      </header>
